...
def prepare_draw_settings(query_file, subject_file, alignments_file, settings):
    ...
    draw_settings['draw_height'] = settings['top_margin'] + settings['main_height'] + settings['bottom_margin']
...
def draw_features(context, settings,  draw_settings, feature_colors, seq_data, y_position, seq_type):
    ...
    rendered = settings.get('features_rendered',
    ...
    for key, feature in features.items():
        ...
        if feature['type'] == 'gene':
            if feature.get('CDS') is not None:
                ...
                for key2, cds in cdss.items():
                    ...
                    if len(cds['exons']) > 0 and 'exon' in rendered:
                        exons = cds['exons']
                    ...
                    elif len(cds['introns']) > 0 and 'intron' in rendered:
                        introns = cds['introns']
        ...
        elif feature['type'] == 'misc_feature' and 'misc_feature' in rendered:
